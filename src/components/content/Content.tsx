...
function Content({ blogs }: ContentProps) {
  ...
  return (
    <Box width={{ xs: "100%", md: "70%" }}>
      {blogs.map(item => (
        <Box key={item.id} onClick={() => router.push(`/blog/${item.slug}`)} sx={{ backgroundColor: "rgba(0, 0, 0, 0.5)", padding: "20px", marginTop: "20px", borderRadius: "8px", boxShadow: "0px 8px 16px rgba(255, 255, 255, 0.1)", cursor: "pointer" }}>
          <Box position={"relative"} width={"100%"} height={{ xs: "35vh", md: "50vh" }}>
          ...
          <Box display={"flex"} gap={"10px"} marginTop={"20px"}>
            ...
            <Box>
              ...
              <Box color={"gray"}>{format(new Date(item.createdAt), 'dd MMM, yyyy')} &#x2022; {calculateReadingTime(item.description.text)}</Box>
            </Box>
          </Box>
        </Box>
      ))}
